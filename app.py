...
if st.button("Calcular ruta") and sel1 and sel2:
    ...
    ry1, rx1 = G.nodes[nodo1]["y"], G.nodes[nodo1]["x"]
    ry2, rx2 = G.nodes[nodo2]["y"], G.nodes[nodo2]["x"]
    folium.Marker([ry1, rx1], tooltip=reverse_geocode(ry1, rx1), icon=folium.Icon(color="green")).add_to(m)
    folium.Marker([ry2, rx2], tooltip=reverse_geocode(ry2, rx2), icon=folium.Icon(color="red")).add_to(m)
